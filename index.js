...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const leaderNamesEl = document.getElementById('leaderNames');
    const femalePlayerNamesEl = document.getElementById('femalePlayerNames');
    ...
    function handleGenerateTeams() {
        ...
        try {
            ...
            const errors = [];
            // --- Validation ---
            if (allPlayerNamesInput.length === 0)
                errors.push("A lista de jogadores não pode estar vazia.");
            if (isNaN(numTeams) || numTeams <= 0)
                errors.push("Número de times inválido. Deve ser maior que zero.");
            if (isNaN(playersPerTeam) || playersPerTeam <= 0)
                errors.push("Número de jogadores por time inválido. Deve ser maior que zero.");
            ...
            let distributableFemales = uniqueFemalePlayerNames.filter(female => !shuffledLeaders.includes(female) && availablePlayers.includes(female));
            ...
            while (distributableFemales.length > 0) {
                ...
                for (const team of teamsSortedByPlayerCount) {
                    ...
                    if (distributableFemales.length === 0)
                        break;
                }
            ...
            while (availablePlayers.length > 0 && safetyNetIterations > 0) {
                ...
                if (!playerAssignedInRound)
                    break; // No team could take more players
                safetyNetIterations--;
            }
            // --- Display Results ---
